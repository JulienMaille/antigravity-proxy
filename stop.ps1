...
if ($proxyPids.Count -gt 0) {
  ...
  # Also try by port
  $portPid = (netstat -ano | Select-String ':4000 ' | ForEach-Object { ($_ -split '\s+')[-1] }) | Where-Object { $_ -ne '0' } | Select-Object -First 1
  if ($portPid) {
    Stop-Process -Id $portPid -Force -ErrorAction SilentlyContinue
    Write-Ok "Killed process on port 4000 (PID $portPid)"
  } else {
